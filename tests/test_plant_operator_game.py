...
class PlantOperatorGameTest(unittest.TestCase):
    ...
    def test_rejects_non_convertible_timeout_with_public_value_error(self):
        for timeout_ms in (None, "abc", True, 0, -1, 1.5, math.inf, math.nan):
    ...
    def test_winning_evidence_requires_every_engineering_check(self):
        evidence = ChallengeEvidence(
            ...
            total_power_kw=4_150.0,
            total_cooling_duty_kw=5_450.0,
            specific_power_kwh_per_tonne=37.7,
        ...
        self.assertEqual(native_check.actual, "No failed or unavailable checks")
        self.assertGreaterEqual(assessment.score, 700)
        self.assertLessEqual(assessment.score, 1000)

    def test_power_and_cooling_limits_are_strict(self):
        evidence = ChallengeEvidence(
            feed_flow_kg_hr=TARGET_FLOW_KG_HR,
            export_pressure_bara=128.0,
            export_temperature_c=45.0,
            stage_1_discharge_temperature_c=70.0,
            stage_2_discharge_temperature_c=70.0,
            total_power_kw=MAX_TOTAL_POWER_KW,
            total_cooling_duty_kw=MAX_COOLING_DUTY_KW,
            specific_power_kwh_per_tonne=38.0,
        ...
        self.assertFalse(assessment.won)
        self.assertEqual(
            {"Compression power", "Cooling-system load"},
            failed,
        )
        checks = {check.name: check for check in assessment.checks}
        self.assertEqual(checks["Compression power"].actual, "4,200.00 kW")
        self.assertEqual(checks["Cooling-system load"].actual, "5,500.00 kW")
